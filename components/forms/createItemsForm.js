  ...
  const domString = `
    <form id="${obj.firebaseKey ? `update-item--${obj.firebaseKey}` : 'submit-item'}" class="mb-4">
      <div class="form-group">
        ...
        <input type="text" class="form-control" id="item-name" aria-describedby="itemName" placeholder="Enter Item name" value="${obj.items.name || ''}" required>
      </div>
      <div class="form-group">
        <label for="priceItem">item price</label>
        <textarea class="form-control" placeholder="" id="item-price" style="height: 100px">${obj.items.price || ''}</textarea>
      </div>
